...
// 主游戏循环（如果game.js中定义了gameLoop，启动它）
function loop() {
...
// 窗口大小改变
window.addEventListener('resize', function() {
  if (typeof game !== 'undefined' && game && game.renderer) {
    game.width = window.innerWidth;
    game.height = window.innerHeight;
    game.renderer.resize(game.width, game.height);
    if (game.garden) game.garden.resize(game.width, game.height);
  }
});
